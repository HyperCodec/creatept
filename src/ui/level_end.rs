use crate::{
    ...
    environment::{
        level_loading::{cleanup, LevelCleanup},
        spawn_cycle::{end_level, EndLevelEvent},
        EnvironmentTime,
    },
...
impl Plugin for LevelEndUIPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            (
                handle_empty_event!(setup_level_end_ui.after(end_level), EndLevelEvent),
                handle_return_click.run_if(|state: Res<GameState>| state.is_menu()),
            ),
        );
    }
}
    ...
    commands
        ...
        .with_children(|parent| {
            ...
            parent
                ...
                .insert(ReturnButton)
                .with_children(|parent| {
                    parent.spawn(TextBundle::from_section("Level Menu", text_style.clone()));
                });
        });
}
