...
use crate::{
    common_assets::{self, CommonAssets},
    environment::level_loading::{LevelCleanup, LevelLoadRequest, LevelManager},
...
impl Plugin for LevelBrowserPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<ReturnToMenuEvent>()
            .add_systems(Startup, setup.after(common_assets::init_assets))
            .add_systems(
                Update,
                (
                    handle_load_click.run_if(|state: Res<GameState>| state.is_menu()),
                    handle_empty_event!(setup.before(handle_return_click), ReturnToMenuEvent),
                ),
            );
    }
}
...
fn setup(common_assets: Res<CommonAssets>, levels: Res<LevelManager>, mut commands: Commands) {
    commands.spawn((Camera2dBundle::default(), MenuCamera, LevelCleanup));
